...
const renderTask=(value,taskid,done)=>{
...
const registerTask = async (task,taskId) => {
    ...
    const dbtask = db.child(`tasks/${taskId}`)
    dbtask.set({
        name: task,
        completed: false,
...
const updateTask =(id,done)=>{
    const db = firebase.database().ref()
    const dbtask = db.child(`tasks/${id}`)
      dbtask.update({
          completed:done
      })
...
const getTasks = () => {
    const db = firebase.database().ref()
    const dbtasks = db.child('tasks')
    dbtasks.once('value', (data) => {
            data.forEach(task =>{
                renderTask(task.val().name,task.key,task.val().completed)
